...
const TOOL_CANDIDATES = Object.freeze([
  ...
  { path: '/maas-zam-hesaplama/', title: 'Maaş Zam Hesaplama', description: 'Eski maaş, yeni maaş veya zam oranından ikisini gir; üçüncü değeri ve aylık-yıllık farkı hesapla.', group: 'salary', keywords: ['maaş zam hesaplama', 'yüzde zam hesaplama maaş', 'maaşım yüzde kaç arttı', 'zamlı maaş'] },
...
function hubPage(tools) {
  ...
  const description = '2026 maaş, zam oranı, asgari ücret, kıdem, ihbar, tazminat, işsizlik maaşı, fazla mesai, yıllık izin, vergi dilimi ve işveren maliyeti hesaplama araçlarını tek sayfadan açın.';
...
export async function applyCalculatorDiscovery(dist) {
  ...
  for (const required of ['/', '/maas-zam-hesaplama/', '/tazminat-hesaplama/', '/kidem-tazminati-hesaplama/', '/ihbar-tazminati-hesaplama/', '/issizlik-maasi-hesaplama/', '/fazla-mesai-hesaplama/', '/yillik-izin-ucreti-hesaplama/', '/asgari-ucret-hesaplama/']) {
  ...
  const homeTools = ['/maas-zam-hesaplama/', '/asgari-ucret-hesaplama/', '/tazminat-hesaplama/', '/kidem-tazminati-hesaplama/', '/ihbar-tazminati-hesaplama/', '/issizlik-maasi-hesaplama/', '/fazla-mesai-hesaplama/', '/yillik-izin-ucreti-hesaplama/', '/isveren-maliyeti-hesaplama/', '/vergi-dilimi-hesaplama/'].map((path) => tools.find((tool) => tool.path === path)).filter(Boolean);
  home = insertHome(home, block('Maaştan çalışan haklarına: diğer hesaplama araçları','Maaş zammı, asgari ücret, kıdem, ihbar, işsizlik, fazla mesai, yıllık izin, vergi dilimi ve işveren maliyeti araçlarına doğrudan geçin.', homeTools, 'calculator-discovery--home')); await writeFile(homeFile, home, 'utf8');

  const contextualTargets = ['/veriler/2026/', '/hesaplama-metodolojisi/', '/sss/', '/sozluk/', '/senaryolar/'];
  const contextualTools = ['/', '/maas-zam-hesaplama/', '/asgari-ucret-hesaplama/', '/tazminat-hesaplama/', '/kidem-tazminati-hesaplama/', '/ihbar-tazminati-hesaplama/', '/issizlik-maasi-hesaplama/', '/fazla-mesai-hesaplama/', '/yillik-izin-ucreti-hesaplama/'].map((path) => tools.find((tool) => tool.path === path)).filter(Boolean);
  let contextualPages = 0;
  for (const route of contextualTargets) {
    if (!(await routeExists(dist, route))) continue;
    const file = distFile(dist, route); let html = await readFile(file, 'utf8'); html = insertBeforeMainEnd(html, block('İlgili hesaplama araçları','Okuduğunuz bilgiyi kendi maaş, zam, asgari ücret veya çalışan hakkı senaryonuzla kontrol edin.', contextualTools, 'calculator-discovery--compact')); await writeFile(file, html, 'utf8'); contextualPages += 1;
  }
