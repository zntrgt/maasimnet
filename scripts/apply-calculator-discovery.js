...
function hubPage(tools) {
  ...
  const description = '2026 maaş, brütten nete, kıdem, ihbar, tazminat, işsizlik maaşı, fazla mesai, vergi dilimi ve işveren maliyeti hesaplama araçlarını tek sayfadan açın.';
...
export async function applyCalculatorDiscovery(dist) {
  ...
  for (const required of ['/', '/tazminat-hesaplama/', '/kidem-tazminati-hesaplama/', '/ihbar-tazminati-hesaplama/', '/issizlik-maasi-hesaplama/', '/fazla-mesai-hesaplama/']) {
  ...
  const homeTools = ['/tazminat-hesaplama/', '/kidem-tazminati-hesaplama/', '/ihbar-tazminati-hesaplama/', '/issizlik-maasi-hesaplama/', '/fazla-mesai-hesaplama/', '/isveren-maliyeti-hesaplama/', '/vergi-dilimi-hesaplama/'].map((path) => tools.find((tool) => tool.path === path)).filter(Boolean);
  home = insertHome(home, block('Maaştan çalışan haklarına: diğer hesaplama araçları','Maaş hesabından sonra kıdem, ihbar, tazminat, işsizlik ödeneği, fazla mesai, vergi dilimi ve işveren maliyeti araçlarına doğrudan geçin.', homeTools, 'calculator-discovery--home')); await writeFile(homeFile, home, 'utf8');

  const contextualTargets = ['/veriler/2026/', '/hesaplama-metodolojisi/', '/sss/', '/sozluk/', '/senaryolar/'];
  const contextualTools = ['/', '/tazminat-hesaplama/', '/kidem-tazminati-hesaplama/', '/ihbar-tazminati-hesaplama/', '/issizlik-maasi-hesaplama/', '/fazla-mesai-hesaplama/'].map((path) => tools.find((tool) => tool.path === path)).filter(Boolean);
  let contextualPages = 0;
  for (const route of contextualTargets) {
    if (!(await routeExists(dist, route))) continue;
    const file = distFile(dist, route); let html = await readFile(file, 'utf8'); html = insertBeforeMainEnd(html, block('İlgili hesaplama araçları','Okuduğunuz bilgiyi kendi maaş, işsizlik, fazla mesai veya tazminat senaryonuzla kontrol edin.', contextualTools, 'calculator-discovery--compact')); await writeFile(file, html, 'utf8'); contextualPages += 1;
  }
  ...
  if (await routeExists(dist, blogRoute)) {
    ...
    const terminationTools = ['/kidem-tazminati-hesaplama/', '/ihbar-tazminati-hesaplama/', '/tazminat-hesaplama/'].map((path) => tools.find((tool) => tool.path === path)).filter(Boolean);
    html = insertBeforeMainEnd(html, block('Okuduktan sonra hesabını yap','Giydirilmiş brüte hangi ödemelerin girdiğini belirlediyseniz şimdi kıdem veya toplam tazminatınızı hesaplayın.', terminationTools, 'calculator-discovery--compact')); await writeFile(file, html, 'utf8'); contextualPages += 1;
  }
