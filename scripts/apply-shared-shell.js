...
export async function applySharedShell(dist) {
  ...
  for (const path of files) {
    ...
    if (!html.includes(SHELL_STYLE_MARKER) || !html.includes('class="site-header"') || !html.includes('class="site-footer"') || !html.includes('data-cookiebot-renew>Çerez Tercihleri')) {
      throw new Error(`Ortak header/footer veya Cookiebot tercih bağlantısı eksik üretildi: ${path}`);
    }
    await writeFile(path, html);
  }
  console.log(`Aynı ortak header/footer HTML, CSS ve Cookiebot tercih bağlantısı uygulandı: ${files.length} sayfa`);
}
