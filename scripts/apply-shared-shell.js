...
export async function applySharedShell(dist) {
  ...
  for (const path of files) {
    ...
    if (!html.includes(SHELL_STYLE_MARKER) || !html.includes('class="site-header"') || !html.includes('class="site-footer"')) {
      throw new Error(`Ortak header/footer eksik üretildi: ${path}`);
    }
    await writeFile(path, html);
  }
  console.log(`Aynı ortak header/footer HTML ve CSS uygulandı: ${files.length} sayfa`);
}